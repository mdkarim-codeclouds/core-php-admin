                    ...
                    <div class="row justify-content-center">
                        <div class="col-lg-4 col-md-6 d-flex flex-column align-items-center justify-content-center">
                            <div class="d-flex justify-content-center py-4">
                                <a href="<?=APP_URL.'/'?>" class="logo d-flex align-items-center w-auto">
                                    ...
                                    <form class="row g-3" novalidate method="POST">
                                        ...
                                        <div class="col-12">
                                            ...
                                            <div class="input-group has-validation">
                                                ...
                                                <input type="email" name="email" class="form-control" id="yourEmail" value="<?= !empty($_SESSION['old']['email']) ? $_SESSION['old']['email'] :'' ?>" required>
                                                <?php if(!empty($_SESSION['errors']['email']) || !empty($_SESSION['errors']['not_found'])){ ?>
                                                <div class="invalid-feedback d-block"><?= ( $_SESSION['errors']['email'] ?? $_SESSION['errors']['not_found'] ) ?></div>
                                                <?php } ?>
                                            </div>
                                        </div>
                                        <div class="col-12">
                                            <label for="yourPassword" class="form-label">Password</label>
                                            <input type="password" name="password" class="form-control" id="yourPassword" value="<?= !empty($_SESSION['old']['password']) ? $_SESSION['old']['password'] :'' ?>" required>
                                            <?php if(!empty($_SESSION['errors']['password'])){ ?>
                                            <div class="invalid-feedback d-block"><?= $_SESSION['errors']['password'] ?></div>
                                            <?php } ?>
                                        </div>
